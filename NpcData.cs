...
namespace EveAbyssCompanion
{
    public static class NpcData
    {
        ...
        public static List<NpcEntry> Build(AppConfig config)
        {
            ...
            // Always resolve relative to actual exe location
            var exeDir = System.IO.Path.GetDirectoryName(System.Diagnostics.Process.GetCurrentProcess().MainModule!.FileName)
                        ?? AppDomain.CurrentDomain.BaseDirectory;
            var datasetPath = config.NpcDatasetPath;
            if (!System.IO.Path.IsPathRooted(datasetPath))
                datasetPath = System.IO.Path.Combine(exeDir, datasetPath);

            if (string.IsNullOrWhiteSpace(datasetPath) || !System.IO.File.Exists(datasetPath))
            {
                LastLoadStatus = $"NPC dataset: built-in defaults (dataset not found: {datasetPath}).";
                return fallback;
            }

            var import = NpcDatasetImporter.TryLoadFromFile(datasetPath, out var imported, out var status);
